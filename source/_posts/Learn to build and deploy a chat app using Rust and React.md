---
title: Learn to build and deploy a chat app using Rust and React
date: '2024-12-11 18:59:51'
...
For this project, we will be deploying via [shuttle](https://shuttle.rs/), which is an open-source, Rust-native cloud development platform that has a generous free tier and provisions resources for you as you need them. If you want an example of what the final code should look like, you can view it [here](https://github.com/joshua-mo-143/react-websocket-chat-rust).

A live deployment link can be found [here](https://websocket-chat-react-rust-example.shuttleapp.rs/) if you'd like to see the result before we start.

![]()

## [Permalink](https://joshmo.hashnode.dev/building-deploying-a-chat-app-with-react-typescript-rust#heading-initial-setup)Initial Setup
...
## [Permalink](https://joshmo.hashnode.dev/building-deploying-a-chat-app-with-react-typescript-rust#heading-getting-started)Getting Started
...
## [Permalink](https://joshmo.hashnode.dev/building-deploying-a-chat-app-with-react-typescript-rust#heading-setting-up-rust)Setting up Rust
...
For the back end part, because we'll be serving the web server through [shuttle](https://github.com/shuttle-hq/shuttle), we will need to install their CLI which we can do with the following command:
...
Once the installation is done, you can start a [shuttle](https://github.com/shuttle-hq/shuttle) project with the following command below (run this in your React project at the packages.json level):
...
```plain
[package]
name = "websocket-chat-react-rust" 
version = "0.1.0"
edition = "2021"
publish = false
...
## [Permalink](https://joshmo.hashnode.dev/building-deploying-a-chat-app-with-react-typescript-rust#heading-getting-started-1)Getting Started
...
## [Permalink](https://joshmo.hashnode.dev/building-deploying-a-chat-app-with-react-typescript-rust#heading-admin-routing)Admin Routing
...
Let's dissect our new additions quickly. `shuttle_secrets::Secrets` allows us to set environmental variables using a Secrets.toml file, much like how you'd typically use a .env file to be able to use environment variables locally and in production. `shuttle_static_folder::StaticFolder` provides a path to our static assets folder so that when we deploy to [shuttle](https://www.shuttle.rs/), the deployment will know where to find our static assets (although we won't implement that just yet).
...
## [Permalink](https://joshmo.hashnode.dev/building-deploying-a-chat-app-with-react-typescript-rust#heading-integrating-front-andamp-back-end)Integrating Front & Back End
...
## [Permalink](https://joshmo.hashnode.dev/building-deploying-a-chat-app-with-react-typescript-rust#heading-finishing-up)Finishing Up
...
Now if we run `npm run deploy`, it should build all of our assets into the required folder and then attempt to deploy to [shuttle](https://www.shuttle.rs/) - assuming there are no issues, it should deploy successfully!
...
If you need to check the status of your [shuttle](https://www.shuttle.rs/) project at any time, you can do so by using `cargo shuttle status` at the Cargo.toml file, or you can add the `--name` flag followed by the project's name to use it from any directory.

Thank you for reading! I hope you managed to take something away from this article. If you'd like to learn more about Rust and get the most out of [shuttle](https://www.shuttle.rs/), you can find their docs [here](https://docs.shuttle.rs/introduction/welcome). There will be more articles coming packed with detailed tutorials on how to create similar web service apps using Typescript & Rust shortly, so make sure to keep a look out!  


> 来自: [Learn to build and deploy a chat app using Rust and React](https://joshmo.hashnode.dev/building-deploying-a-chat-app-with-react-typescript-rust)
>
